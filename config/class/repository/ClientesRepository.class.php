...
class ClientesRepository {
    public function gravar($clientes) {
        try {
            $pdo = ConexaoDB();
            if($clientes->getId() > 0) {
            $comando = $pdo->prepare("UPDATE clientes SET cli_nome = :cli_nome, cli_dtnasc = :cli_dtnasc, cli_sexo = :cli_sexo, cli_cpf = :cli_cpf, cli_email = :cli_email, cli_senha = :cli_senha, cli_data_cadastro = :cli_data_cadastro, cli_tipo = :cli_tipo WHERE cli_id = :cli_id");
